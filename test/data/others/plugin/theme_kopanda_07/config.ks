...
;=========================================
	...
	tf.btn_path_on1 = tf.img_path + 'set1.png';
	tf.btn_path_on2 = tf.img_path + 'set2.png';

	tf.btn_path_mute_bgm = tf.img_path + 'bgmmute.png';
	tf.btn_path_mute_se  = tf.img_path + 'semute.png';
	...
	// チェック画像の座標
	tf.config_x = [816, 317, 359, 401, 443, 482, 524, 565, 607, 648, 689];
	tf.config_y = [154, 226, 298, 370];
...
;------------------------------------------------------------------------------------------------------
;	未読スキップ-ON
	[button name="unread_on"  fix="true" target="*skip_on"  graphic="&tf.btn_path_off" width="152" height="52" x="304" y="437"]

;	未読スキップ-OFF
	[button name="unread_off" fix="true" target="*skip_off" graphic="&tf.btn_path_off" width="152" height="52" x="464" y="437"]
...
*load_bgm_img
	...
	[image layer="0" name="bgmvol" storage="&tf.btn_path_mute_bgm" x="&tf.config_x[0]" y="&tf.config_y[0]"]
	[else]
	[image layer="0" name="bgmvol" storage="&tf.btn_path_on1" x="&tf.config_x[tf.config_num_bgm]" y="&tf.config_y[0]"]
	[endif]
	[return]

*load_se_img
	[if exp="tf.config_num_se == 0"]
	[image layer="0" name="sevol"  storage="&tf.btn_path_mute_se" x="&tf.config_x[0]" y="&tf.config_y[1]"]
	[else]
	[image layer="0" name="sevol"  storage="&tf.btn_path_on2" x="&tf.config_x[tf.config_num_se]" y="&tf.config_y[1]"]
	[endif]
	[return]

*load_ch_img
	[image layer="0" name="ch" storage="&tf.btn_path_on1" x="&tf.config_x[tf.config_num_ch + 1]" y="&tf.config_y[2]"]
	[return]

*load_auto_img
	[image layer="0" name="auto"  storage="&tf.btn_path_on2" x="&tf.config_x[tf.config_num_auto + 1]" y="&tf.config_y[3]"]
	[return]

*load_skip_img
	[if exp="tf.text_skip == 'ON'"]
	[image layer="0" name="skip" storage="&tf.img_path_skip_on"  x="304" y="437"]
	[else]
	[image layer="0" name="skip" storage="&tf.img_path_skip_off" x="464" y="437"]
	[endif]
	[return]
